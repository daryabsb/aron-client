<template>
  <div>
    ...
    <TransitionRoot as="template" :show="true">
      <TransitionChild as="template" enter="ease-out duration-2000" enter-from="opacity-0" enter-to="opacity-100"
        leave="ease-in duration-2000" leave-from="opacity-100" leave-to="opacity-0">
                  ...
                  <form action="#" class="sm:mx-auto sm:max-w-xl lg:mx-0">
                    <div class="sm:flex">
                      <div class="min-w-0 flex-1">
                        ...
                        <input id="search-orders" type="text" placeholder="Enter a customer name or an order number"
                    ...
                    <div class="bg-aronium-800 px-16 overflow-hidden overflow-x-auto shadow sm:rounded-sm mt-3">
                      <table class="divide-y divide-gray-500">
                        <thead>
                          <tr>
                            <p class="my-3 text-sm text-gray-300 sm:my-4">
                              Pending orders:
                            </p>
                            <!--     <th
                        ...
                        <tbody v-if="cart.length > 0" class="divide-y divide-gray-500 text-white">
                          <tr v-for="order in cart
                          .filter((o) => !o.status)
                          .slice(0, 5)" :key="order.id">
                            <td class="w-full max-w-0 whitespace-nowrap px-2 py-2 text-sm">
                              <div class="flex">
                                <!-- :href="order.href" -->

                                <span class="cursor-pointer group inline-flex space-x-2 truncate text-sm"
                                  @click="openOrder(order)">
                                  <BanknotesIcon class="h-5 w-5 flex-shrink-0 group-hover:text-gray-500"
                                    aria-hidden="true" />
