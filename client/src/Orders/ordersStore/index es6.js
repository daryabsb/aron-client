...
export const useOrderStore = defineStore("orders", () => {
  ...
  // STATE
  const cart = ref(useStorage("cart", []));
  const activeNumber = ref(useStorage("activeNumber", ""));
  const activeItem = ref(useStorage("activeItem", {}));
  // const activeOrder = ref(useStorage("activeOrder", {}));
  const paymentTypes = ref(useStorage("paymentTypes", []));
  ...
  const createCartFromAPI = async () => {
    ...
    try {
      ...
      cart.value = [...cart.value, ...newOrders].unique();
  ...
  function setActiveItem(item) {
    ...
    console.log("activeItem set from store", activeItem.value);
